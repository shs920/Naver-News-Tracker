...
async def send_modified(old: dict, new: dict, url: str, press: str, version: int, img_changed: bool):
    changes = []
...
async def handle_deleted(url: str):
    ...
    if not rows or rows[0].get("is_deleted"):
        return
    record = rows[0]
